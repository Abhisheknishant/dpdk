...
void qede_rx_queue_release(void *rx_queue)
{
	...
	if (rxq) {
		/* This function is shared for CMT and regular mode,
		 * and we provide struct qede_fastpath_cmt in CMT mode and
		 * struct qede_rx_queue in regular mode to DPDK app.
		 * Only way to detect if we are in CMT mode is by reading
		 * qdev element, hence both structures have qdev as first
		 * element so dereferencing becomes easy using rxq structure in
		 * even if we are in CMT mode.
		 */
		qdev = rxq->qdev;
