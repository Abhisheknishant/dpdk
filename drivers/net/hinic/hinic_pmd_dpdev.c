...
static void hinic_osdep_deinit(hinic_nic_dev *nic_dev)
{
	...
	if (paddr_hash) {
		...
		while (rte_hash_iterate(paddr_hash, (const void **)&key_pa,
					(void **)&data_mz, &iter) >= 0) {
			if (data_mz) {
				PMD_DRV_LOG(WARNING, "Free leaked dma_addr: 0x%lx, mz: %s",
					    key_pa, data_mz->name);
			   ...
			   unsigned int flag, unsigned int align)
{
	...
	rc = rte_hash_lookup_with_hash(nic_dev->os_dep->dma_addr_hash,
				       &iova, sig);
	if (rc >= 0) {
		PMD_DRV_LOG(ERR, "Dma addr: 0x%lx already in hash table, error: %d, mz_name: %s",
			    iova, rc, z_name);
	...
	if (rc) {
		PMD_DRV_LOG(ERR, "Insert dma addr: 0x%lx hash failed, error: %d, mz_name: %s",
			    iova, rc, z_name);
		goto phys_addr_hash_err;
	}
	*dma_handle = iova;
...
void hinic_dma_mem_free(void *dev, size_t size, void *virt, dma_addr_t phys)
{
	...
	if (rc < 0) {
		PMD_DRV_LOG(ERR, "Can not find phys_addr: 0x%lx, error: %d",
			    phys, rc);
		return;
	}
