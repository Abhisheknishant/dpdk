			...
			int main_fd __rte_unused)
{
	...
	if ((dev->flags & VIRTIO_DEV_BUILTIN_VIRTIO_NET) &&
		...
		while (dev->nr_vring > 2) {
			...
			cleanup_vq(vq, 1);
			cleanup_vq_inflight_split(dev, vq);
			free_vq(dev, vq);
		}
	}
		...
		int main_fd __rte_unused)
{
	...
	if (vq_is_packed(dev)) {
		/* TODO */
		RTE_LOG(ERR, VHOST_CONFIG,
			"Don't support the packed ring\n");
		return RTE_VHOST_MSG_RESULT_ERR;
	} else {
		mmap_size = num_queues * get_pervq_shm_size_split(queue_size);
	}
...
vhost_check_queue_inflights_split(struct virtio_net *dev, struct vhost_virtqueue *vq)
{
	...
	if (vq->inflight_split->used_idx != used->idx) {
		vq->inflight_split->desc[vq->inflight_split->last_inflight_io].inflight = 0;
		rte_compiler_barrier();
		vq->inflight_split->used_idx = used->idx;
	}
	...
	if (resubmit_num) {
		...
		for (i = 0; i < vq->inflight_split->desc_num; i++) {
			if (vq->inflight_split->desc[i].inflight == 1) {
				...
				resubmit->resubmit_list[resubmit->resubmit_num].counter =
					vq->inflight_split->desc[i].counter;
		...
		vq->resubmit_inflight_split = resubmit;
	}
