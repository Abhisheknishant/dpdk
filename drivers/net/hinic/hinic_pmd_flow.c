			...
			struct hinic_5tuple_filter *filter)
{
	...
	PMD_DRV_LOG(INFO, "Add 5tuple succed, type: 0x%x, qid: 0x%x, enable: 0x%x",
	...
	case IPPROTO_TCP:
		if (filter->filter_info.dst_port == RTE_BE16(BGP_DPORT_ID)) {
			...
			PMD_DRV_LOG(INFO, "Set dport bgp succed, qid: 0x%x, enable: 0x%x",
				filter->queue,
				filter_info->pkt_filters[i].enable);
		} else if (filter->filter_info.src_port ==
			RTE_BE16(BGP_DPORT_ID)) {
			...
			PMD_DRV_LOG(INFO, "Set sport bgp succed, qid: 0x%x, enable: 0x%x",
					filter->queue,
					filter_info->pkt_filters[i].enable);
		}
	...
	case IPPROTO_VRRP:
		...
		PMD_DRV_LOG(INFO, "Set VRRP succed, qid: 0x%x, enable: 0x%x",
				filter->queue,
				filter_info->pkt_filters[i].enable);
		break;
			   ...
			   struct hinic_5tuple_filter *filter)
{
	...
	PMD_DRV_LOG(INFO, "Del 5tuple succed, type: 0x%x, qid: 0x%x, enable: 0x%x",
			       ...
			       bool add)
{
	...
	if (add) {
		...
		if (ret_fw) {
			...
			(void)hinic_ethertype_filter_remove(filter_info, i);
			return -ENOENT;
		}
		PMD_DRV_LOG(INFO, "Add ethertype succed, type: 0x%x, qid: 0x%x, enable: 0x%x",
				filter_info->pkt_type, filter->queue,
				filter_info->pkt_filters[i].enable);
		...
		case RTE_ETHER_TYPE_SLOW:
			...
			PMD_DRV_LOG(INFO, "Add lacp tcam succed");
			break;
		default:
			break;
		}
		...
		if ((filter_info->type_mask & (1 << i))) {
			...
			PMD_DRV_LOG(INFO, "Del ethertype succed, type: 0x%x, qid: 0x%x, enable: 0x%x",
					filter_info->pkt_type,
					filter_info->pkt_filters[i].qid,
					filter_info->pkt_filters[i].enable);
			  ...
			  bool add)
{
	...
	if (add) {
		...
		PMD_DRV_LOG(INFO, "Add fdir filter succed, flag: 0x%x, qid: 0x%x, key: 0x%x",
				fdir_info.fdir_flag, fdir_info.qid,
				fdir_info.fdir_key);
	} else {
		ret = hinic_set_normal_filter(nic_dev->hwdev, fdir_info.qid,
		...
		PMD_DRV_LOG(INFO, "Del fdir filter succed, flag: 0x%x, qid: 0x%x, key: 0x%x",
				fdir_info.fdir_flag, fdir_info.qid,
				fdir_info.fdir_key);
	}
					...
					struct rte_flow_error *error)
{
	...
	if (!ret) {
		...
		if (!ret) {
			...
			PMD_DRV_LOG(INFO, "Create flow ntuple succed, func_id: 0x%x",
	...
	if (!ret) {
		...
		if (!ret) {
			...
			PMD_DRV_LOG(INFO, "Create flow ethertype succed, func_id: 0x%x",
					hinic_global_func_id(nic_dev->hwdev));
			return flow;
		}
		goto out;
	...
	if (!ret) {
		...
		if (!ret) {
			...
			PMD_DRV_LOG(INFO, "Create flow fdir rule succed, func_id : 0x%x",
					hinic_global_func_id(nic_dev->hwdev));
			return flow;
		}
		goto out;
			      ...
			      struct rte_flow_error *error)
{
	...
	PMD_DRV_LOG(INFO, "Destroy flow succed, func_id: 0x%x",
			hinic_global_func_id(nic_dev->hwdev));
