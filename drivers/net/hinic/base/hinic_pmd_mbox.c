				...
				struct mbox_msg_info *msg_info)
{
	...
	spin_lock(&func_to_func->msg_send_lock);
...
send_err:
	spin_unlock(&func_to_func->msg_send_lock);
			...
			u32 timeout)
{
	...
	spin_lock(&func_to_func->mbox_send_lock);
...
send_err:
	if (err && out_size)
		*out_size = 0;
	spin_unlock(&func_to_func->mbox_send_lock);
			...
			u8 cmd, void *buf_in, u16 in_size)
{
	...
	spin_lock(&hwdev->func_to_func->mbox_send_lock); /*pmd use spinlock*/
	...
	spin_unlock(&hwdev->func_to_func->mbox_send_lock);
...
static int hinic_func_to_func_init(struct hinic_hwdev *hwdev)
{
	...
	spin_lock_init(&func_to_func->mbox_send_lock);
	spin_lock_init(&func_to_func->msg_send_lock);
