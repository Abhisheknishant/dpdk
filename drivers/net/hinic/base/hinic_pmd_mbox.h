...
/* PFs does not support enable SR-IOV cap when PFs use PMD, VFs just receive
...
struct hinic_send_mbox {
	u8 *data;
	volatile u64 *wb_status; /* write back status */
	void *wb_vaddr;
	dma_addr_t wb_paddr;
};
...
struct hinic_mbox_func_to_func {
	...
	spinlock_t mbox_send_lock; /* lock for mbox cmd sending */
	spinlock_t msg_send_lock; /* lock for mbox segments sending */
