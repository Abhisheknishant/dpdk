...
static int
set_surprise_link_check_aer(struct ifpga_rawdev *ifpga_rdev);
...
static int
ifpga_pci_find_ext_capability(unsigned int fd, int cap)
{
	return ifpga_pci_find_next_ext_capability(fd, 0, cap);
}

static int
ifpga_get_dev_vendor_id(const char *bdf, uint32_t *dev_id,
		uint32_t *vendor_id)
{
	int fd;
	char path[1024];
...
static int
ifpga_rawdev_fill_info(struct ifpga_rawdev *ifpga_dev, const char *bdf)
{
...
static int
ifpga_monitor_sensor(struct rte_rawdev *raw_dev, bool *gsd_start)
{
...
static int
fme_clear_warning_intr(struct opae_manager *mgr)
{
	u64 val;
...
static int
ifpga_register_fme_interrupt(struct opae_manager *mgr)
{
	int ret;
...
static int
ifpga_rawdev_get_string_arg(const char *key __rte_unused,
...
ifpga_cfg_probe(struct rte_vdev_device *dev)
{
	...
	ret = rte_eal_hotplug_add(RTE_STR(IFPGA_BUS_NAME), dev_name,
			devargs->args);
end:
	if (kvlist)
