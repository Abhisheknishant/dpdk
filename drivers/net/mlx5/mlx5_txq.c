 ...
 * the total WQE amount may exceed the hardware capabilities. If thw
 ...
 *
 * @return
 *   Zero on success, otherwise the parameters can not be adusted.
 */
static int
txq_adjust_params(struct mlx5_txq_ctrl *txq_ctrl)
{
	...
	if (txq_ctrl->txq.inlen_send > max_inline) {
		DRV_LOG(WARNING,
			"default txq_inline_max is adjusted (%u->%u) due to"
			" large Tx queue size is requested on port %u",
			txq_ctrl->txq.inlen_send, max_inline,
			priv->dev_data->port_id);
		txq_ctrl->txq.inlen_send = max_inline;
	}
	if (txq_ctrl->txq.inlen_empw > max_inline) {
		DRV_LOG(WARNING,
			"default txq_inline_mpw is adjusted (%u->%u) due to"
			" large Tx queue size is requested on port %u",
			txq_ctrl->txq.inlen_empw, max_inline,
			priv->dev_data->port_id);
		txq_ctrl->txq.inlen_empw = max_inline;
